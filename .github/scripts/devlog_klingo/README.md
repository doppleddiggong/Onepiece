...
**캐싱 시스템 (v3.0)**:
- 파일 기반 GPT 응답 캐싱
- 동일한 변경사항에 대해 API 호출 생략
- TTL 24시간 (자동 만료)
- **효과**: API 호출 ~70% 감소

**gpt-4o-mini 기본 모델 (v3.0)**:
- 비용 효율적인 모델로 변경
- 품질은 유사하나 비용 50% 절감
- 필요 시 `gpt-4o`로 변경 가능
...
**총 비용 절감**: v1.0 대비 **약 88% 절감** (v2.0: -60%, v3.0 모델: -50%, v3.0 캐싱: -70%)

### 모델 비교

| 모델 | 비용 | 품질 | 속도 | 권장 용도 |
|------|------|------|------|-----------|
| **gpt-4o-mini** | $0.15/1M tok | ⭐⭐⭐⭐ | 빠름 | **기본 사용 (권장)** |
| gpt-4o | $2.50/1M tok | ⭐⭐⭐⭐⭐ | 보통 | 고품질 필요 시 |
...
```json
{
  "gpt_model": "gpt-4o-mini"  // 또는 "gpt-4o"
}
```

#### 2. 캐싱 설정 (`gpt_client.py`)
